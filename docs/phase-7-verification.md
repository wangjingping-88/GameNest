...
- 0.2.4 客户端内置 `GAMENESTPUBLIC` 生产受信公钥；GitHub Release 仍必须同时配置对应私钥 Secret、公钥 Variable 和 `keyId` Variable，缺任一项即拒绝发布。
- 0.1.0 没有更新客户端；`v0.2.0` 因 SQLite 依赖安全告警被 CI 拒绝，`v0.2.1` 因签名配置尚未完成而未创建 Release，`v0.2.2` 因更新器临时目录测试清理竞争未创建 Release，`v0.2.3` 因工作流未传递 Runner .NET 路径给签名脚本而未创建 Release。均须手动安装 0.2.4；真实在线升级以 0.2.4 到 0.2.5 验证。
...
- 便携更新集成测试的临时目录清理会对 Windows 文件句柄和安全扫描进行有限重试；超过 2 秒仍被占用时测试保持失败，不会强制终止任何进程。
- `scripts/Publish-Portable.ps1`：待 0.2.4 Release 验收时生成 x64 self-contained 便携目录和 ZIP，并执行包内容审计。
