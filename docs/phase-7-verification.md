...
- 0.1.0 没有更新客户端，必须手动安装 0.2.1；`v0.2.0` 因 SQLite 依赖安全告警被 CI 拒绝且未创建 Release。真实在线升级需在生产公钥嵌入且 0.2.1 正式发布后，以 0.2.2 验证。
...
- `scripts/Publish-Portable.ps1`：通过，生成 0.2.1 x64 self-contained 便携目录和 ZIP；包内容审计通过。
